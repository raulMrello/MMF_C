...
void* Fifo::push(void* data, uint16_t *size) throw (Exception){
    ...
    pMsg = (uint8_t *)((uint32_t)_mem + _pwrite);

     // write size bytes
    for(i = 0; i < *size; i++){
    	_mem[_pwrite++] = *(uint8_t*)((uint32_t)data+i);
        pushed++;
        if(_pwrite >= _size)
        	_pwrite = 0;
        if(--_free == 0){
...
void* Fifo::pop(void* data, uint16_t *size) throw (Exception){
    ...
    pMsg = (uint8_t *)((uint32_t)_mem + _pread);

    // extract size bytes
   for(i = 0; i < *size; i++){
	   *(uint8_t*)((uint32_t)data+i) = _mem[_pread++];
       poped++;
       if(_pread >= _size)
       	_pread = 0;
       if(++_free == _size){
...
void Fifo::popSection(void* section, uint16_t size) throw (Exception){
	...
	if(section != (void *)((uint32_t)_mem + _pread) || size > (_size - _pread)){
		PLATFORM_EXIT_CRITICAL();
